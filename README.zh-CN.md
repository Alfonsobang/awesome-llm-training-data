...
- [Finance Preference Review Schema](schemas/finance-preference-review.schema.json) - 面向金融偏好/反馈数据的多维 review schema。
...
- [Harbor 风格金融任务模板](examples/financial-agent-eval-seed/harbor-template) - 面向合规拒答、精确查数、报表依据解释、金融工具轨迹审计、预测 cutoff 检查、风险计算和玩具回测的可迁移任务脚手架，使用合成 fixture、JSON 证据和确定性 verifier。
